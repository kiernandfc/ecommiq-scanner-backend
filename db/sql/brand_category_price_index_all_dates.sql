...
-- DROP VIEW IF EXISTS brand_category_price_index_all_dates;

-- Create the brand category price index view for all dates from 2025-04-14
CREATE OR REPLACE VIEW brand_category_price_index_all_dates AS
WITH search_categories AS (
    ...
    FROM
        competitor_weighted_price_all_dates -- Changed from _14day
    WHERE 
...
brand_category_summary AS (
    ...
    SELECT
        ...
        -- Count the number of competitors contributing to the average
        COUNT(CASE WHEN NOT is_reference_brand THEN 1 ELSE NULL END) AS competitor_count
    FROM
        search_categories
    GROUP BY
...
COMMENT ON VIEW brand_category_price_index_all_dates IS 
'Provides a price index comparing reference brand prices to average competitor prices within each category, using data from 2025-04-01 onwards.
Includes competitor_count representing the number of competitors factored into the avg_competitor_price.
