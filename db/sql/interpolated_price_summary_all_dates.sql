...
-- DROP MATERIALIZED VIEW IF EXISTS interpolated_price_summary_all_dates;

-- Create the materialized view for interpolated summaries from 2025-04-01 to current date
CREATE MATERIALIZED VIEW interpolated_price_summary_all_dates AS
WITH date_series AS (
    -- Generate dates from 2025-04-01 through the current date
    SELECT generate_series(
               '2025-04-01'::date,             -- Start date
               CURRENT_DATE,                     -- End date (today)
               '1 day'::interval                 -- Step interval
           )::date AS report_date
),
-- Get distinct products that have at least 3 datapoints in the daily_price_summary_all_dates
distinct_products AS (
    SELECT 
        catalog_id
    FROM 
        daily_price_summary_all_dates -- Changed from daily_price_summary
    WHERE 
        summary_date >= '2025-04-01'::date -- Changed date filter
    GROUP BY 
        catalog_id
    HAVING 
...
product_date_combinations AS (
    -- Create all product x date combinations for the specified window
    SELECT
        p.catalog_id,
        d.report_date
...
daily_data AS (
    SELECT * FROM daily_price_summary_all_dates -- Changed from daily_price_summary
    WHERE 
        summary_date >= '2025-04-01'::date -- Changed date filter
...
CREATE INDEX IF NOT EXISTS idx_interpolated_price_summary_all_dates_cat_date
ON interpolated_price_summary_all_dates (catalog_id, report_date);

-- Note: Remember to refresh this view periodically, potentially after refreshing the daily_price_summary_all_dates view
-- REFRESH MATERIALIZED VIEW interpolated_price_summary_all_dates;
