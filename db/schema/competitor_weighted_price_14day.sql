...
latest_review_counts AS (
    -- Get the most recent review count for each product
    SELECT 
        catalog_id,
    ...
    FROM 
        interpolated_price_summary_14day
    WHERE 
...
weighted_prices AS (
    -- Join the interpolated price data with the mappings and calculate weights
    SELECT
        rm.competitor_id,
        rm.catalog_id,
        rm.competitor_brand,
    ...
    JOIN
        interpolated_price_summary_14day ips ON rm.catalog_id = ips.catalog_id
    LEFT JOIN
...
CREATE INDEX IF NOT EXISTS idx_competitor_weighted_price_14day
ON competitor_weighted_price_14day (competitor_id, report_date);

-- Additional index for brand and query-based lookups
CREATE INDEX IF NOT EXISTS idx_competitor_weighted_price_14day_brand_query
ON competitor_weighted_price_14day (competitor_brand, reference_brand, search_query, report_date);

-- Note: Remember to refresh this view periodically, after refreshing the interpolated_price_summary_14day view
